...
GUI_POLL_INTERVAL_VISIBLE = 0.05
GUI_POLL_INTERVAL_BACKGROUND = 0.20
    ...
    async def ask_login(self) -> LoginData:
        ...
        while True:
            ...
            if (
                ...
                or not re.match(r'^[a-zA-Z0-9_]+$', login_data.username)
            ):
...
class ChannelList:
    ...
    def configure_theme(self, *, watching_bg: str, watching_fg: str) -> None:
        self._table.tag_configure("watching", background=watching_bg, foreground=watching_fg)
...
class TrayIcon:
    ...
    def minimize(self):
        if sys.platform == "darwin":
            return
        if self.available and self.icon is None:
            self._start()
        if self.icon is not None:
    ...
    def change_icon(self, state: str):
        if state not in {"pickaxe", "active", "idle", "error", "maint"}:
...
class GUIManager:
    def __init__(self, twitch: Twitch):
        ...
        if self._twitch.settings.tray and sys.platform != "darwin" and self.tray.available:
            # NOTE: this starts the tray icon thread
            self._root.after_idle(self.tray.minimize)
        else:
            self._root.after_idle(self._root.deiconify)

    def _configure_platform_fonts(self) -> None:
        if sys.platform != "win32":
            return
        fonts = {
            "TkDefaultFont": {"family": "Segoe UI", "size": 9},
            "TkTextFont": {"family": "Segoe UI", "size": 9},
            "TkMenuFont": {"family": "Segoe UI", "size": 9},
            "TkHeadingFont": {"family": "Segoe UI Semibold", "size": 9},
            "TkFixedFont": {"family": "Consolas", "size": 9},
        }
        for name, config in fonts.items():
            try:
                nametofont(name).configure(**config)
            except tk.TclError:
                continue
    ...
    async def _poll(self):
        ...
        while True:
            ...
            if state in {"withdrawn", "iconic"}:
                interval = GUI_POLL_INTERVAL_BACKGROUND
            else:
                interval = GUI_POLL_INTERVAL_VISIBLE
    ...
    def apply_theme(self, dark: bool) -> None:
        ...
        s.configure("Large.TButton", font=self._fonts["large"], padding=(8, 4))
        ...
        self._fonts["monospaced"] = nametofont("TkFixedFont").copy()
        self._fonts["monospaced"].config(size=10)
        s.configure("MS.TLabel", font=self._fonts["monospaced"])
        s.configure("green.TLabel", foreground=success)
        s.configure("yellow.TLabel", foreground=warning)
        s.configure("red.TLabel", foreground=danger)
        ...
        s.map(
            "TButton",
            background=[("active", header), ("pressed", border), ("disabled", bg)],
        ...
        s.map(
            "TNotebook.Tab",
            background=[("selected", header), ("active", fieldbg)],
            foreground=[("disabled", muted)],
        )
        # Entries/Combos
        s.configure(
        ...
        s.configure(
            ...
            foreground=fg,
            bordercolor=border,
            rowheight=max(24, default_font.metrics("linespace") + 8),
        )
        s.map(
            "Treeview",
        ...
        self.channels.configure_theme(watching_bg=watching_bg, watching_fg=watching_fg)
        ...
        if dark:
            self._set_title_bar_color(0x00202020)
        else:
