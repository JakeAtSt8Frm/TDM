        ...
        async with self._lock:
            if url in self._hashes:
                ...
                if img_hash in self._images:
                    ...
                    try:
                        with Image_module.open(CACHE_PATH / img_hash) as cached_image:
                            self._images[img_hash] = image = cached_image.copy()
            ...
            try:
                async with self._twitch.request("GET", url) as response:
                    if response.status != 404:
                        with Image_module.open(io.BytesIO(await response.read())) as raw_image:
