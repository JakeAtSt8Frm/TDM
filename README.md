...
This application allows you to AFK mine timed Twitch drops, without having to worry about switching channels when the one you were watching goes offline, claiming the drops, or receiving the stream data itself. This helps you save on bandwidth and hassle.

This native build is tuned for regular desktop use, with persistent settings stored in the local `config` folder and runtime cache files stored next to the application.
